...
STATE_FILE = os.path.join(SCRIPT_DIR, 'bnb_bn_state.json')
...
def log_trade(entry):
    ...
    if action == 'OPEN':
        lines += [
            f'操作: 开仓{d_cn}',
            f'数量: {entry.get("qty")} BNB | 杠杆: {entry.get("leverage")}x',
...
class Analyzer:
    ...
    def plan(self):
        ...
        if d == 'SHORT':
            ...
            bnbme = levels[0][1] if levels else 'market'
...
class Executor:
    ...
    def close_position(self, position_side):
        try:
            ...
            log(f'平仓: {position_side} {amt} BNB')
            cr = {
                'action': 'CLOSE',
                'symbol': 'BNB/USDT',
            ...
            if '-4045' in err:
                ...
                try:
                    ...
                    if isinstance(all_algos, list):
                        for aa in all_algos:
                            try:
                                p3 = sign2({'symbol': 'BNBUSDT', 'algoId': aa['algoId']})
        ...
        try:
            ...
            for pos in self.ex.fetch_positions([SYMBOL]):
                ...
                for a in active_algos:
                    ...
                    if not has_match:
                        try:
                            p3 = signed({'symbol': 'BNBUSDT', 'algoId': a['algoId']})
...
def main():
    log('══════ BNB自动交易 启动 (币安 10x) ══════')
    log(f'品种: {SYMBOL}  仓位: {POSITION_SIZE} BNB  轮询: {POLL_SECONDS}s')
                    ...
                    try:
                        ...
                        if os.path.exists(epath):
                            with open(epath) as _f:
                                edata = _json.load(_f)
                                enrich = edata.get('coins', {}).get('BNB', {})
                        ...
                        if decision == 'CONFIRMED':
                            ...
                            _write_trade_log('BNB', 'CONFIRMED', reason, {'direction': direction, 'entry_price': plan['entry'], 'stop_loss': plan['sl'], 'take_profit': plan['tp'], 'qty': POSITION_SIZE, 'leverage': LEVERAGE})
                            executor.open_position(plan)
                            state['last_signal'] = direction
                            save_state(state)
                        else:
                            log(f'LLM否决: {reason[:60]}')
                            _write_trade_log('BNB', 'REJECTED', reason, {'direction': direction, 'entry_price': plan['entry'], 'stop_loss': plan['sl'], 'take_profit': plan['tp'], 'qty': POSITION_SIZE, 'leverage': LEVERAGE})
                    except Exception as e:
                        log(f'LLM异常({e})，安全拒绝开仓')
                        _write_trade_log('BNB', 'REJECTED', f'LLM异常: {str(e)[:60]}', {'direction': direction, 'entry_price': plan['entry'], 'stop_loss': plan['sl'], 'take_profit': plan['tp'], 'qty': POSITION_SIZE, 'leverage': LEVERAGE})
